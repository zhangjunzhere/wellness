...
public abstract class InstantAnswersAdapter extends BaseAdapter implements ViewGroup.OnHierarchyChangeListener, OnItemClickListener {
    ...
    protected State state = State.DETAILS; //Ed modify
    ...
    protected View getCacheView(int position) {
        View cacheView = viewArray.get(position);
        if (cacheView != null) {
            EditText editText = (EditText) cacheView.getTag();
            if (editText != null) {
                editText.clearFocus();
    ...
    protected void setCacheView(int position,View view) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        View cacheView = getCacheView(position);
        if (cacheView!=null){
            return cacheView;
        }
        ...
        if (view == null) {
            if (type == LOADING) {
                ...
                restoreEnteredText(textField, field, "",position);
                textField = field;
                /*
        ...
        if (type == BUTTON) {
            ...
            Utils.displayInstantAnswer(context, view, (BaseModel) getItem(position));
            ...
            if (type == EMAIL_FIELD) {
                ...
                restoreEnteredText(emailField, field, Session.getInstance().getEmail(),position);
                emailField = field;
                field.setHint(R.string.uv_email_address_hint);
                field.setInputType(EditorInfo.TYPE_TEXT_VARIATION_EMAIL_ADDRESS|EditorInfo.TYPE_TEXT_FLAG_NO_SUGGESTIONS);
            } else if (type == NAME_FIELD) {
                title.setText(R.string.uv_your_name);
                restoreEnteredText(nameField, field, Session.getInstance().getName(),position);
                nameField = field;
                field.setHint(R.string.uf_sdk_name_hint);
                field.setInputType(EditorInfo.TYPE_TEXT_VARIATION_PERSON_NAME|EditorInfo.TYPE_TEXT_FLAG_NO_SUGGESTIONS);
    ...
    protected void restoreEnteredText(EditText previous,final EditText current, String defaultText, final int position) {

        current.setInputType(EditorInfo.TYPE_TEXT_FLAG_NO_SUGGESTIONS);
        current.setOnFocusChangeListener(new OnFocusChangeListener() {

            @Override
            public void onFocusChange(View v, boolean hasFocus) {
                current.setCursorVisible(false);
                if (viewArray.get(position)!=null){
                    viewArray.get(position).setTag(current);
                }

                if (!systemSetText && hasFocus){

...
public class ValuePie extends ValueAbs {
    ...
    public void drawValue(Resources resources, Canvas canvas, Paint paint) {
        ...
        for(int i = 0; i < data.length; i++) {
            ...
            canvas.drawText(legendValueText, pieObj.center.x + (radius + pieObj.bias[i]) * (float) Math.cos(radian), pieObj.center.y + (radius + pieObj.bias[i]) * (float) Math.sin(radian) + bounds.height() / 2, paint);

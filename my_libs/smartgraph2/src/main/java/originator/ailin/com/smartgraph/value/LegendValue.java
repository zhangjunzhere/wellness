...
public class LegendValue {
    ...
    public void drawValuePie(PieObj pieObj, float radius, float[] data) {
        ...
        for(int i = 0; i < data.length; i++) {
            ...
            mCanvas.drawText(legendValueText, pieObj.center.x + (radius + pieObj.bias[i] - mPieLegendValueMargin) * (float) Math.cos(radian), pieObj.center.y + (radius + pieObj.bias[i] - mPieLegendValueMargin) * (float) Math.sin(radian) + bounds.height() / 2, mPaint);
    ...
    public void drawValuePieRose(PieObj pieObj, float[] radiusDealArray, float[] data) {
        ...
        for(int i = 0; i < data.length; i++) {
            ...
            mPaint.getTextBounds(legendValueText, 0, legendValueText.length(), bounds);
            double radian = Math.PI * (2 * startAngleInit + swipeAngle) / 360;
            mCanvas.drawText(legendValueText, pieObj.center.x + (radiusDealArray[i] - mPieRoseLegendValueMargin) * (float) Math.cos(radian), pieObj.center.y + (radiusDealArray[i] - mPieRoseLegendValueMargin) * (float) Math.sin(radian) + bounds.height() / 2, mPaint);

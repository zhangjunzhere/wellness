...
public class ValueRose extends ValueAbs {
    ...
    public void drawValue(Resources resources, Canvas canvas, Paint paint) {
        ...
        for(int i = 0; i < data.length; i++) {
            ...
            canvas.drawText(legendValueText, pieObj.center.x + (radiusDealArray[i] - pieRoseLegendValueMargin) * (float) Math.cos(radian), pieObj.center.y + (radiusDealArray[i] - pieRoseLegendValueMargin) * (float) Math.sin(radian) + bounds.height() / 2, paint);

...
/**
 * Copy of packages/apps/UnifiedEmail - com.android.mail.ui.SwipeHelper with
 * changes.
 */
public class SwipeHelper {
    ...
    private static final boolean LOG_SWIPE_DISMISS_VELOCITY = false; // STOPSHIP
                                                                     // DEBUG
                                                                     // ONLY
    ...
    public static float ALPHA_FADE_START = 0f; // fraction of thumbnail width where fade starts
    static final float ALPHA_FADE_END = 0.7f; // fraction of thumbnail width beyond which alpha->0
    ...
    public SwipeHelper(Context context, int swipeDirection, Callback callback, float densityScale, float pagingTouchSlop) {
        mCallback = callback;
        ...
        if (SWIPE_ESCAPE_VELOCITY == -1) {
            ...
            SWIPE_ESCAPE_VELOCITY = res.getInteger(R.integer.asus_commonui_swipe_escape_velocity);
            DEFAULT_ESCAPE_ANIMATION_DURATION = res.getInteger(R.integer.asus_commonui_escape_animation_duration);
            MAX_ESCAPE_ANIMATION_DURATION = res.getInteger(R.integer.asus_commonui_max_escape_animation_duration);
            MAX_DISMISS_VELOCITY = res.getInteger(R.integer.asus_commonui_max_dismiss_velocity);
            SNAP_ANIM_LEN = res.getInteger(R.integer.asus_commonui_snap_animation_duration);
            DISMISS_ANIMATION_DURATION = res.getInteger(R.integer.asus_commonui_dismiss_animation_duration);
            SWIPE_SCROLL_SLOP = res.getInteger(R.integer.asus_commonui_swipe_scroll_slop);
            MIN_SWIPE = res.getDimension(R.dimen.asus_commonui_min_swipe);
            MIN_VERT = res.getDimension(R.dimen.asus_commonui_min_vert);
            MIN_LOCK = res.getDimension(R.dimen.asus_commonui_min_lock);
        }
    }
    ...
    public static void invalidateGlobalRegion(View view, RectF childBounds) {
        ...
        while (view.getParent() != null && view.getParent() instanceof View) {
            ...
            if (DEBUG_INVALIDATE) {
                Log.v(TAG, "INVALIDATE(" + (int) Math.floor(childBounds.left) + "," + (int) Math.floor(childBounds.top)
                        + "," + (int) Math.ceil(childBounds.right) + "," + (int) Math.ceil(childBounds.bottom));
            }
        }
    }
    ...
    private void dismissChild(final View view, float velocity) {
        // to avoid delete other while the user operate quickly
        if (!mDelFin || oldView == view)
            return;
        mDelFin = false;
        oldView = view;
    ...
    public void snapChild(final View view, float velocity) {
        ...
        anim.addListener(new Animator.AnimatorListener() {
            ...
            public void onAnimationEnd(Animator animation) {
                ...
                mCallback.onDragCancelled(view);
            }
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        case MotionEvent.ACTION_MOVE:
            if (mCurrView == null || mCurrView.getId() == 0) {
                break;
            } else {
        ...
        case MotionEvent.ACTION_CANCEL:
            if (mCurrView != null) {
                ...
                boolean childSwipedFarEnough = DISMISS_IF_SWIPED_FAR_ENOUGH && translation > 0.4 * currAnimViewSize;
                ...
                if (LOG_SWIPE_DISMISS_VELOCITY) {
                    Log.v(TAG, "Swipe/Dismiss: " + velocity + "/" + escapeVelocity + "/" + perpendicularVelocity
                            + ", x: " + translation + "/" + currAnimViewSize);
                }
